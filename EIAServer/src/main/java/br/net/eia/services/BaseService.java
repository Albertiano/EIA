...
import br.net.eia.persistence.GenericDao;
...
public abstract class BaseService<T extends BaseEntity & HATEOASEntity, C extends EntityCollection<T>> {
	...
	public Response create(T entity) {
		...
		return Response.created(
				UriBuilder.fromPath("/" + entity.getId()).build()).build();
	}
	...
	@GET
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public C retrieve(@QueryParam("page") int page, @Context UriInfo uriInfo) {
		...
		if (page < totalPages) {
			try {
				collection
						.addLink(buildPageLink(page + 1, "nextPage", uriInfo));
			} catch (IllegalArgumentException | UriBuilderException e) {
				// TODO Auto-generated catch block
				Logger.getLogger(getClass().getName()).log(
					Level.ERROR, e.getLocalizedMessage(), e);
		...
		if (page > 0) {
			try {
				collection
						.addLink(buildPageLink(page - 1, "prevPage", uriInfo));
			} catch (IllegalArgumentException | UriBuilderException e) {
				// TODO Auto-generated catch block
				Logger.getLogger(getClass().getName()).log(
					Level.ERROR, e.getLocalizedMessage(), e);
	...
	@Path("/all")
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public C retrieveAll() {
		Collection<T> entities = dao.retrieve();
		for (T entity : entities) {
			entity.createStandardLinks();
		}
		C collection = encapsulateEntities(entities);
	...
	private Link buildPageLink(int pageNumber, String rel, UriInfo uriInfo) {
		try {
			return new Link(
					URLDecoder.decode(
							UriBuilder
									.fromPath(
											uriInfo.getPath() + "?page="
													+ pageNumber).build()
									.toString(), "UTF-8"), rel);
